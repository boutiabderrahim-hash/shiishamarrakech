...
const App: React.FC = () => {
  ...
  // Handlers
  const toggleLang = () => setLang(prev => (prev === 'ar' ? 'es' : 'ar'));
  ...
  const handleOpenDrawer = () => {
    const open = () => {
      const newTransaction: Transaction = {
          ...
          amount: 0,
          timestamp: new Date().toISOString(),
          description: `Cash drawer opened by ${currentWaiter?.name}`,
          tax: 0,
      };
      setTransactions(prev => [...prev, newTransaction]);
      triggerCashDrawer();
    ...
    if (currentWaiter?.role === 'WAITER') {
        setPinRequest({
            ...
            onConfirm: (pin: string) => {
              if (pin === pins.ADMIN || pin === pins.MANAGER) {
                setPinRequest({ isOpen: false });
                open();
              } else {
                alert(t('incorrectPin'));
    ...
    } else { // Manager or Admin is already logged in
        open();
    }
  };
